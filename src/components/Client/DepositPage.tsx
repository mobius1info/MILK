...
export default function DepositPage({ userId, onBack, onSuccess }: DepositPageProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (depositAmount < selectedMethod.min_amount) {
      alert(`Minimum deposit amount is $${selectedMethod.min_amount.toFixed(2)}`);
      return;
    }

    if (selectedMethod.max_amount && depositAmount > selectedMethod.max_amount) {
      alert(`Maximum deposit amount is $${selectedMethod.max_amount.toFixed(2)}`);
      return;
    }

    if (!transactionHash.trim()) {
      alert('Please enter transaction hash or reference');
      return;
    }
