...
import { Wallet, TrendingUp, TrendingDown, Clock, Check, X, Users, Copy, Package, FileText, User, Home, UserPlus, Headphones, FileCheck, Info, HelpCircle, DollarSign, CheckCircle, XCircle, Crown, ShoppingBag } from 'lucide-react';
...
interface DashboardProps {
  ...
  initialTab?: 'overview' | 'deposit' | 'orders' | 'referrals' | 'transactions' | 'profile' | 'vip' | 'vip-categories';
}
...
export default function Dashboard({ profile, onBalanceUpdate, initialTab = 'overview' }: DashboardProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'deposit' | 'orders' | 'referrals' | 'transactions' | 'profile' | 'vip' | 'vip-categories'>(initialTab);
  ...
  useEffect(() => {
    const titles: Record<typeof activeTab, string> = {
      ...
      'transactions': 'ML MALL - Transactions',
      'profile': 'ML MALL - Profile',
      'vip': 'ML MALL - VIP',
      'vip-categories': 'ML MALL - VIP Categories'
    };
  ...
  return (
    <div className="pb-24">
      ...
      {activeTab === 'vip' && <VIPPurchase />}

      {activeTab === 'vip-categories' && <VIPCategories />}
