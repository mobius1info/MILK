...
import { Package, Clock, CheckCircle, XCircle, Crown, Lock, Wallet, AlertCircle, ShoppingCart, ListTodo } from 'lucide-react';
...
export default function OrderHistory({ userId, onNavigateToDeposit }: OrderHistoryProps) {
  ...
  const [activeTasksModal, setActiveTasksModal] = useState<{ categoryId: string; vipLevel: number } | null>(null);
  ...
  const isVIPLevelAccessible = (level: number) => {
    ...
    return categoryAccess[vipLevel.category] === true;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 border-b">
        ...
        {vipLevels.map((vip) => {
          const hasCategoryAccess = categoryAccess[vip.category] === true;
      ...
      <div className="space-y-4">
        {vipLevels.filter(vip => {
          ...
          const approvedVIPPurchase = vipPurchases.find(
            p => p.vip_level === vip.level && p.category_id === vip.category && p.status === 'approved'
          );
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {approvedVIPPurchase && (
                      <button
                        onClick={() => setActiveTasksModal({ categoryId: vip.category, vipLevel: vip.level })}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                  <div className="bg-gradient-to-br from-green-50 to-green-100/30 p-4 rounded-lg border border-green-200">
                    <p className="text-xs text-gray-600 mb-1 uppercase tracking-wide">Your Commission Rate</p>
                    <p className="text-2xl font-bold text-green-600">{vip.commission.toFixed(0)}%</p>
                  </div>
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100/30 p-4 rounded-lg border border-blue-200">
                    <p className="text-xs text-gray-600 mb-1 uppercase tracking-wide">Category</p>
                ...
                {!hasCategoryAccess && vip.category && (() => {
                  const categoryPrice = categoryPrices[vip.category] || 0;
      ...
      {activeTasksModal && (
        <ActiveTasksModal
          userId={userId}
          categoryId={activeTasksModal.categoryId}
          vipLevel={activeTasksModal.vipLevel}
          onClose={() => setActiveTasksModal(null)}
        />
      )}
    </div>
  );
