/*
  # Fix combo deposit - actually credit on correct position
  
  1. Changes
    - Count how many products purchased for this VIP purchase
    - Check if current purchase matches combo position from snapshot
    - If yes, credit combo deposit based on VIP price and combo percentage
    - Record combo_deposit transaction
  
  2. Formula
    - Combo deposit = vip_price * (combo_deposit_percent_at_approval / 100)
    - Example: $100 VIP * 300% = $300 combo deposit
*/
...
BEGIN
  ...
  IF v_vip_purchase.combo_enabled_at_approval = true 
    ...
    VALUES (
      ...
      v_combo_deposit,
      'combo_deposit',
      format('Combo bonus at position %s (%.0f%%)', v_current_position, v_vip_purchase.combo_deposit_percent_at_approval),
      'completed'
    );
