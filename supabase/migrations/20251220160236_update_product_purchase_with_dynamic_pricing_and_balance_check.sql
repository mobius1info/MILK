/*
  # Update Product Purchase with Dynamic Pricing and Balance Check

  1. Changes
    - Calculate product price dynamically based on VIP price:
      * Regular products (not 9th): 30%-100% of VIP price
      * COMBO products (9th, 18th, etc): 100%-200% of VIP price
    - Check user balance before allowing 9th product purchase
    - Require deposit if balance insufficient
    - Return detailed error with required deposit amount

  2. COMBO Product Logic
    - Every 9th product (9, 18, 27...) is a COMBO
    - COMBO price: VIP_price to 2x VIP_price
    - COMBO commission: 3x normal commission
    - Requires balance >= product price to purchase

  3. Error Messages
    - Shows product price and 3x commission info
    - Shows exact amount needed to deposit
*/
...
BEGIN
  ...
  VALUES (
    ...
    v_final_commission,
    'approved',
    format('%sCommission earned: %s (VIP %s)', 
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE 
      ...
      WHEN v_is_ninth_product THEN format('КОМБО товар куплен за $%.2f! Комиссия x3: $%.2f зачислена на баланс', v_product_price, v_final_commission)
      ELSE format('Комиссия $%.2f зачислена на баланс', v_final_commission)
    END
  );
END;
$$;