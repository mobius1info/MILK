/*
  # Fix approve_vip_purchase - Add vip_combo_settings Insert
  
  The function was missing the code to insert into vip_combo_settings table
  when combo is enabled, causing the combo settings to not be saved properly.
  
  This migration updates the function to:
  1. Store combo settings in vip_purchases as snapshot
  2. Store combo settings in vip_combo_settings for the specific VIP
*/
...
BEGIN
  ...
  -- STEP 2: Approve the new VIP purchase and save combo settings
  UPDATE vip_purchases
  SET
    status = 'approved',
    approved_at = now(),
  ...
  -- STEP 4: Create combo settings if enabled
