/*
  # Fix COMBO - only check balance, do NOT deduct
  
  1. Changes
    - Check if user has required balance for combo
    - DO NOT deduct balance
    - Credit base commission × combo_multiplier
    - Return error if insufficient balance
  
  2. Formula
    - Required balance check: VIP price × combo_deposit_percent%
...
BEGIN
  ...
  -- Calculate base commission (same for all products)
  ...
  IF v_vip_purchase.combo_enabled_at_approval = true 
    ...
    v_required_balance := v_vip_purchase.amount_paid * (v_vip_purchase.combo_deposit_percent_at_approval / 100.0);
