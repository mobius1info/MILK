/*
  # Update COMBO Logic to Use Profile Settings

  1. Changes
    - Read combo_product_position from profiles table instead of vip_levels
    - Read combo_multiplier from profiles table instead of vip_levels
    - Read combo_deposit_percent from profiles table
    - Calculate required deposit based on combo_deposit_percent
    - COMBO deposit is now % of VIP price (e.g., 50% of $800 = $400 required)

  2. Logic
    - If combo_deposit_percent > 0, client must deposit that % of VIP price to continue
    - Each client can have different COMBO settings controlled by admin
    - Settings are in profiles table: combo_product_position, combo_multiplier, combo_deposit_percent
*/
...
BEGIN
  ...
  -- Get user profile with combo settings
  SELECT * INTO v_user_profile
  FROM profiles
  WHERE id = v_user_id;
  
  v_user_balance := v_user_profile.balance;
  v_combo_enabled := COALESCE(v_user_profile.combo_enabled, false);
  v_combo_position := COALESCE(v_user_profile.combo_product_position, 9);
  v_commission_multiplier := COALESCE(v_user_profile.combo_multiplier, 3);
  v_combo_deposit_percent := COALESCE(v_user_profile.combo_deposit_percent, 50);
  ...
  -- Check if this is a COMBO product based on user's combo settings AND eligibility
  v_is_combo_product := (v_purchased_count % v_combo_position = 0) 
    AND v_combo_enabled 
    AND (v_user_profile.vip_completions_count >= 1);
