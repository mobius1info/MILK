/*
  # Fix Commission Calculation - Use Product Price

  Changes the commission calculation logic from VIP-based to product-based:
  - Old: Commission = VIP Price × VIP Commission %
  - New: Commission = Product Price × Product Commission %
  
  This makes each product earn different commission based on its individual price.
  
  Examples:
  - Coffee Maker ($89.99 × 15%) = $13.50
  - Blender ($149.99 × 15%) = $22.50
  - Stand Mixer ($299.99 × 15%) = $45.00
*/
...
BEGIN
  ...
  -- NEW: Calculate commission from PRODUCT price, not VIP price
  v_commission_amount := (v_product.price * v_product.commission_percentage / 100.0);
