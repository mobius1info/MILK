/*
  # Rollback to December 23, 2023 16:00 UTC (19:00 MSK)

  1. Changes
    - Drop all tables, functions, and policies created after 16:00 UTC
    - Restore process_product_purchase to version from 20251223105718
    - Remove rejection_reason column from transactions
    - Restore transactions status constraint
    - Remove all combo activation features
    - Remove all withdrawal approval/rejection features
    - Remove all admin policies for product_progress and product_purchases

  2. Security
    - All RLS policies from after 16:00 UTC will be removed
    - System will be restored to state before withdrawal management features

  3. Data Impact
    - Any data in vip_combo_activations table will be PERMANENTLY DELETED
    - Any transaction rejections will be converted to 'pending'
    - Any combo overrides will be LOST
*/

-- Drop all functions created after 16:00 UTC
DROP FUNCTION IF EXISTS admin_add_vip_combo_activation CASCADE;
...
DROP FUNCTION IF EXISTS admin_add_vip_combo_override CASCADE;
DROP FUNCTION IF EXISTS approve_withdrawal CASCADE;

-- Drop all tables created after 16:00 UTC
DROP TABLE IF EXISTS vip_combo_activations CASCADE;
DROP TABLE IF EXISTS vip_purchase_combo_overrides CASCADE;

-- Drop policies created after 16:00 UTC
DROP POLICY IF EXISTS "Admin can view all product progress" ON product_progress;
DROP POLICY IF EXISTS "Admin can view all product purchases" ON product_purchases;
DROP POLICY IF EXISTS "Admins can update all transactions" ON transactions;

-- Convert rejected transactions to pending before changing constraint
...
BEGIN
  ...
  -- Get progress for THIS SPECIFIC VIP PURCHASE
  SELECT * INTO v_progress
  FROM product_progress
  WHERE user_id = v_user_id
    AND vip_purchase_id = v_vip_purchase.id;

  -- Create new progress if not found for this VIP purchase
  IF NOT FOUND THEN
    INSERT INTO product_progress (user_id, vip_level, category_id, vip_purchase_id)
    VALUES (v_user_id, p_vip_level, p_category_id, v_vip_purchase.id)
    RETURNING * INTO v_progress;
  END IF;

  v_purchased_count := v_progress.products_purchased + 1;

  -- Check if all products completed FOR THIS VIP PURCHASE
  ...
  -- COMBO occurs only ONCE at specific position
  v_is_combo_product := (v_purchased_count = v_combo_position) AND v_combo_enabled;
  ...
  IF v_is_combo_product THEN
    -- COMBO product: Fixed price based on Deposit %
    v_product_price := v_required_deposit;
    v_requires_deposit := (v_combo_deposit_percent > 0);

    -- Check if user has enough balance for COMBO product
    IF v_requires_deposit AND v_user_balance < v_product_price THEN
      ...
      RETURN jsonb_build_object(
        'error', format('COMBO product! Price: $%s, Commission: x%s ($%s). You need $%s to purchase this product. Missing: $%s',
  ...
  -- Calculate commission using formula: VIP price * commission_percentage / 25 tasks
  v_base_commission := (v_vip_price * v_vip_commission_percentage / 100.0) / v_total_products;
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE
      ...
      WHEN v_is_combo_product THEN format('COMBO product purchased! Commission x%s: $%s credited to balance', v_commission_multiplier, round(v_final_commission, 2))
