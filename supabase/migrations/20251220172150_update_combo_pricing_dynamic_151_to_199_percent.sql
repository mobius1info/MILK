/*
  # Update COMBO Product Pricing to Dynamic 151-199% of VIP Price

  1. Changes
    - Change COMBO product price from fixed 2x to dynamic range 151% to 199% of VIP price
    - Keep x3 commission multiplier for COMBO products
    - Maintain balance check requirement

  2. Pricing Logic
    - Regular products: 30% to 100% of VIP price (random)
    - COMBO products (every 9th): 151% to 199% of VIP price (random)
...
BEGIN
  ...
  IF v_is_ninth_product THEN
    -- COMBO product: 151% to 199% of VIP price (dynamic)
    v_product_price := v_vip_price * (1.51 + random() * 0.48);
    ...
    IF v_user_balance < v_product_price THEN
      ...
      RETURN jsonb_build_object(
        'error', 'Данный товар КОМБО стоит $' || round(v_product_price, 2)::text || 
                 ' и комиссия за него x3 ($' || round(v_final_commission, 2)::text || 
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE 
      ...
      WHEN v_is_ninth_product THEN 'КОМБО товар куплен за $' || round(v_product_price, 2)::text || 
                                    '! Комиссия x3: $' || round(v_final_commission, 2)::text || ' зачислена на баланс'
