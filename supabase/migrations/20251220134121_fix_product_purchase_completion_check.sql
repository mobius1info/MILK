/*
  # Fix Product Purchase Completion Check
  
  ## Overview
  This migration updates the `process_product_purchase` function to properly check
  if all products in a VIP level have been completed and return the `is_completed` flag.
  
  ## Changes
  1. Add logic to check total products count from vip_levels
  2. Compare purchased count with total count
  3. Return `is_completed` flag in the response
  
  ## Purpose
  - Fix issue where users can purchase beyond the limit of products
  - Show proper completion message when all products are done
  - Prevent infinite product purchases
*/
