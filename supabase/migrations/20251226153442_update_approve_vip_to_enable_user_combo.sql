/*
  # Update approve_vip_purchase to enable user combo

  Adds STEP 4 to update user's profile combo settings when admin approves VIP with combo enabled.
  This ensures that when admin enables combo at approval, it activates in user's profile.
*/
...
BEGIN
  ...
  -- STEP 4: Update user's profile combo settings
  -- When admin enables combo at approval, activate it in user's profile
  UPDATE profiles
  SET 
    combo_enabled = p_combo_enabled,
    combo_product_position = p_combo_position,
