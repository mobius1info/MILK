/*
  # Fix COMBO - Remove VIP Completions Requirement

  1. Changes
    - COMBO now works immediately if combo_enabled = true
    - Remove requirement for vip_completions_count >= 1
    - Combo logic: enabled by admin = works right away
  
  2. Logic
    - If admin enables combo_enabled for client, combo works on matching positions
    - No need to complete VIP first before combo activates
    - vip_completions_count still tracks completions but doesn't affect combo eligibility
*/
...
BEGIN
  ...
  -- Check if this is a COMBO product: matching position AND combo enabled (no completion requirement)
