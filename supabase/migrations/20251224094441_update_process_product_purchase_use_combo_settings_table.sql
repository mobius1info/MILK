/*
  # Update process_product_purchase to use vip_combo_settings table

  1. Changes
    - Update process_product_purchase function to check vip_combo_settings table
    - Check if current product index has an active combo
    - Use combo settings from vip_combo_settings instead of combo_enabled_at_approval
    - Mark combo as completed after use

  2. Notes
    - This allows multiple combos per VIP purchase
    - Each combo can be independently configured and tracked
    - Backward compatible with existing VIP purchases
*/
...
BEGIN
  ...
  VALUES (
    ...
    v_product.price,
    'completed',
    'Product purchase: ' || v_product.name
  );
