/*
  # Update VIP Purchase Completed Count from Product Progress
  
  ## Overview
  This migration updates the `process_product_purchase` function to sync the
  `vip_purchases.completed_products_count` field with the actual progress
  from `product_progress` table.
  
  ## Changes
  1. Update completed_products_count in vip_purchases after each purchase
  2. This keeps the count in sync for display in the UI
  
  ## Purpose
  - Show accurate progress in VIP Categories and Orders
  - Ensure consistency between product_progress and vip_purchases
*/
