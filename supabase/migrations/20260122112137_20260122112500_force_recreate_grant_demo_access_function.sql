/*
  # Force Recreate grant_demo_access Function

  1. Problem
    - Old versions of function may still reference product_progress table

  2. Solution
    - Drop all versions of grant_demo_access function
    - Recreate with clean implementation that only uses existing tables

  3. Security
    - SECURITY DEFINER to allow admin operations
    - Check admin role from auth.users metadata
*/

-- Drop all versions of the function
DROP FUNCTION IF EXISTS grant_demo_access(text, uuid);
DROP FUNCTION IF EXISTS grant_demo_access(uuid, uuid);

-- Recreate function with clean implementation
...
BEGIN
  ...
  -- Check if user already has ACTIVE demo access for this level
  ...
  IF existing_demo_purchase IS NOT NULL THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'User already has active demo access for this VIP level'
    );
  END IF;
  ...
  -- Create new VIP purchase with demo access (amount_paid = 0 marks it as demo)
  ...
  VALUES (
    ...
    0, -- Zero amount marks this as demo access
    vip_level_data.products_count,
    0,
    'approved', -- Auto-approve demo access
    false,
    false,
    NULL,
    NULL,
  ...
  RETURN jsonb_build_object(
    ...
    'vip_level', vip_level_data.name,
    'category', vip_level_data.category,
    'products_count', vip_level_data.products_count
  );
END;
$$;