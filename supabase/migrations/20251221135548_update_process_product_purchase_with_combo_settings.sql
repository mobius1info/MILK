...
BEGIN
  ...
  IF v_is_combo_product THEN
    ...
    IF v_user_balance < v_product_price THEN
      ...
      RETURN jsonb_build_object(
        'error', format('Данный товар КОМБО стоит $%s и комиссия за него x%s ($%s). Для покупки и продолжения выполнения задач вы должны пополнить счет на недостающую сумму: $%s', 
  ...
  VALUES (
    ...
    format('%sCommission earned: %s (VIP %s - %s%%)', 
      CASE WHEN v_is_combo_product THEN 'КОМБО x' || v_commission_multiplier || ' - ' ELSE '' END,
      v_product.name, 
      p_vip_level,
      v_vip_commission_percentage)
  );
  ...
  RETURN jsonb_build_object(
    ...
    'message', CASE 
      ...
      WHEN v_is_combo_product THEN 'КОМБО товар куплен за $' || round(v_product_price, 2) || '! Комиссия x' || v_commission_multiplier || ' (' || v_vip_commission_percentage || '%): $' || round(v_final_commission, 2) || ' зачислена на баланс'
