/*
  # Fix COMBO Product - No Balance Deduction

  1. Changes
    - COMBO product now only CHECKS balance, but does NOT deduct money
    - User must have the required amount on balance to pass the check
    - After check passes, only commission is added (no money is deducted)

  2. Rationale
    - COMBO product is a "balance verification" checkpoint
    - It ensures user has reached a certain balance level
    - But the money stays in user's balance
    - User still earns commission (x3 multiplier)
*/
...
BEGIN
  ...
  -- REMOVED: No balance deduction for COMBO products
  -- Balance is only checked, not deducted
