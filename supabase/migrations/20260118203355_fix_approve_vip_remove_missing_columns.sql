/*
  # Fix approve_vip_purchase function - remove missing columns

  1. Problem
    - Function tries to update combo_* columns in profiles table that don't exist
    - This causes constraint error: "combo_multiplier_check"
    
  2. Solution
    - Remove UPDATE profiles statement for combo columns
    - Keep only vip_combo_settings logic
    
  3. Note
    - Combo settings are stored in vip_combo_settings table
    - Profiles table doesn't need these columns
*/
...
BEGIN
  ...
  -- STEP 4: Create vip_combo_settings record if combo is enabled
